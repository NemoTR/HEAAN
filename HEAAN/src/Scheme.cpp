...
namespace heaan
{
    ...
    void Scheme::addEncKey(SecretKey &secretKey)
    {
        ...
        long np = ceil((1 + logQQ + logN + 2) / (double)pbnd); // 如何求得的？
        ring.sampleUniform2(ax, logQQ);
        ring.mult(bx, secretKey.sx, ax, np, QQ);
        ring.subFromGaussAndEqual(bx, QQ); // bx = (ex - (ax * sx)) % QQ
    ...
    void Scheme::addMultKey(SecretKey &secretKey)
    {
        ...
        ring.sampleUniform2(ax, logQQ); // ax[i] in [0, 2^QQ)
        ring.mult(bx, secretKey.sx, ax, np, QQ);
        ring.subFromGaussAndEqual(bx, QQ); // bx = ex - (ax * sx)

        np = ceil((2 + logN + 2) / (double)pbnd);
        ring.mult(sxsx, secretKey.sx, secretKey.sx, np, Q); // sxsx = sx^2 % Q
        ring.leftShiftAndEqual(sxsx, logQ, QQ);				// sxsx = sxsx * Q % QQ
        ring.addAndEqual(bx, sxsx, QQ);						// bx = (ex - (ax * sx) + sx^2 * Q) % QQ
        delete[] sxsx;
    ...
    void Scheme::addConjKey(SecretKey &secretKey)
    {
        ...
        ZZ *sxconj = new ZZ[N];
        ring.conjugate(sxconj, secretKey.sx); // sxconj = K_(-1)(sx)
        ring.leftShiftAndEqual(sxconj, logQ, QQ);
        ring.addAndEqual(bx, sxconj, QQ); // bx = (ex - (ax * sx) + sxconj * Q) % QQ
        delete[] sxconj;
    ...
    void Scheme::addLeftRotKey(SecretKey &secretKey, long r)
    {
        ...
        ZZ *spow = new ZZ[N];
        ring.leftRotate(spow, secretKey.sx, r); // spow = K_(5^r)(sx)
        ring.leftShiftAndEqual(spow, logQ, QQ);
        ring.addAndEqual(bx, spow, QQ); // bx = (ex - (ax * sx) + spow * Q) % QQ
        delete[] spow;
    ...
    void Scheme::encryptMsg(Ciphertext &cipher, Plaintext &plain)
    {
        ...
        cipher.n = plain.n;
        ZZ qQ = ring.qpows[plain.logq + logQ]; // qQ = q * Q = 2^(logq + logQ)
        ...
        // 为什么在这里而不在上面，或者为什么是 qQ，根据论文此处有误。
        ring.addGaussAndEqual(cipher.ax, qQ);
        ring.addGaussAndEqual(cipher.bx, qQ);
    }
    ...
    void Scheme::encryptBySk(Ciphertext &cipher, SecretKey &secretKey, complex<double> *vals, long n, long logp, long logq, double sigma1)
    {
        ...
        ring.subFromGaussAndEqual(cipher.bx, QQ, sigma1); // bx = (ex - (ax * sx)) % QQ

        ring.addAndEqual(cipher.bx, plain.mx, QQ); // 此处为什么是 QQ 而不是 qQ？疑有误。参考第 309 行。

        ring.rightShiftAndEqual(cipher.ax, logQ);
        ring.rightShiftAndEqual(cipher.bx, logQ);

        // 没有附加误差，参考第 319、320 行
    }
    ...
    // cipher = cipher / i = cipher * -i (乘以虚数 -i)
    ...
    void Scheme::normalizeAndEqual(Ciphertext &cipher)
    {
        ...
        for (long i = 0; i < N; ++i)
        {
            if (NumBits(cipher.ax[i]) == cipher.logq)
                cipher.ax[i] -= q;
            if (NumBits(cipher.bx[i]) == cipher.logq)
                cipher.bx[i] -= q;
        }
    }
